...
- **The library** (`packages/terrarium`, npm workspace, published as `@terrariumlabs/core`, imported as `@terrariumlabs/core/*`): `scenario.ts` (`defineScenario`
  ...
  `inject.ts` (postMessage bridge, EIP-6963 "Terrarium Wallet", `window.terrarium`, installs the interceptor), `devbar.ts` (plain-DOM overlay: controls, Hide, transaction explorer fed by `terrarium_transactions`, decoded with `ctx.label(address, name, abi)`, the scenario's `abis` and `known-abi.ts`), `vite-plugin.js` (+ `.d.ts`) (generates `.terrarium/{inject,worker}.ts`
