...
**In Node**, `runScenario` from `@terrariumlabs/core/worker` boots the scenario without a page, and `terrarium_http` works the
same way; `test/unit/http.test.mjs` does this, and also exercises the page side by installing the interceptor on a fake
`fetch`. The parser is exported from `@terrariumlabs/core/http` (`parseGraphql`) if you want to unit-test a resolver's inputs.
...
| `reply(body, { status?, headers? })` from `@terrariumlabs/core/scenario` | an explicit answer; a string body is `text/plain`, anything else JSON |
...
| `@terrariumlabs/core/http` | `parseGraphql(source, variables?, operationName?)`, `compileMatcher`, `installHttpInterceptor(provider, routesPromise, scope?)`, `runRoute(ctx, route, raw)` for custom hosts and tests |
