...
end in the same call, `startTerrarium(worker)` from `@terrarium/core/inject`; they differ in who makes that call and whether
...
| Next.js, Remix, CRA, any React app not on Vite | [`@terrarium/react`](#%EF%B8%8F-react-component-terrariumreact) | yes, behind a build-time guard |
...
`terrarium_*` methods. Full surface: [api.md](api.md#terrariumreact).
...
```js
// next.config.js
module.exports = { transpilePackages: ['@terrarium/core', '@terrarium/react', '@terrarium/evm'] };
```
